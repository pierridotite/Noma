#!/usr/bin/env python3
"""
XOR with PyTorch torch.compile (Inductor).
Measures compile overhead separately from training time.
...
def main() -> int:
    out_dir = Path(sys.argv[1]) if len(sys.argv) > 1 else ROOT / "out" / "runs" / "latest" / "torch_compile"
    ...
    compile_overhead_ms = (time.perf_counter() - compile_start) * 1000
    ...
    while step < max_iter:
        ...
        if phase == 1 and step >= growth_step - 1:
            ...
            # Create new larger model (need to recompile)
            ...
            # Recompile for new architecture
            try:
                compiled_model = torch.compile(model, mode="reduce-overhead")
                _ = compiled_model(X)  # Warmup
            except Exception:
                compiled_model = model
    ...
    print(f"[torch_compile] {step} iters, final_loss={loss_records[-1][1]:.6f}, total={total_ms:.2f}ms, compile={compile_overhead_ms:.2f}ms")
    return 0
